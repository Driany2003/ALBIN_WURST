...
public interface VentaRepository extends JpaRepository<VentaEntity, Integer> {
  ...
  @Query(value = "SELECT " +
      "(SELECT COALESCE(SUM(v.ven_total), 0) " +
      " FROM T_VENTA v " +
      " WHERE CAST(v.ven_fecha AS DATE) BETWEEN :fechaInicio AND :fechaFin) AS precioVentaTotal, " +

      "(SELECT COALESCE(SUM(vd.ven_det_cantidad * p.pro_precio_costo), 0) " +
      " FROM T_VENTA v " +
      " JOIN T_VENTA_DETALLE vd ON v.ven_id = vd.venta_id " +
      " JOIN T_PRODUCTO p ON vd.producto_id = p.pro_id " +
      " WHERE CAST(v.ven_fecha AS DATE) BETWEEN :fechaInicio AND :fechaFin) AS precioCostoTotal, " +

      "(COALESCE((SELECT SUM(v.ven_total) " +
      "           FROM T_VENTA v " +
      "           WHERE CAST(v.ven_fecha AS DATE) BETWEEN :fechaInicio AND :fechaFin), 0) " +
      " - COALESCE((SELECT SUM(vd.ven_det_cantidad * p.pro_precio_costo) " +
      "             FROM T_VENTA v " +
      "             JOIN T_VENTA_DETALLE vd ON v.ven_id = vd.venta_id " +
      "             JOIN T_PRODUCTO p ON vd.producto_id = p.pro_id " +
      "             WHERE CAST(v.ven_fecha AS DATE) BETWEEN :fechaInicio AND :fechaFin), 0)) AS ganancia, " +

      "(SELECT COALESCE(COUNT(DISTINCT v.ven_id), 0) " +
      " FROM T_VENTA v " +
      " WHERE CAST(v.ven_fecha AS DATE) BETWEEN :fechaInicio AND :fechaFin) AS totalVentas, " +

      "(SELECT COALESCE(SUM(CASE WHEN v.ven_tipo_pago = 'Yape' THEN v.ven_total ELSE 0 END), 0) " +
      " FROM T_VENTA v " +
      " WHERE CAST(v.ven_fecha AS DATE) BETWEEN :fechaInicio AND :fechaFin) AS totalYape, " +

      "(SELECT COALESCE(SUM(CASE WHEN v.ven_tipo_pago = 'Plin' THEN v.ven_total ELSE 0 END), 0) " +
      " FROM T_VENTA v " +
      " WHERE CAST(v.ven_fecha AS DATE) BETWEEN :fechaInicio AND :fechaFin) AS totalPlin, " +

      "(SELECT COALESCE(SUM(CASE WHEN v.ven_tipo_pago = 'Efectivo' THEN v.ven_total ELSE 0 END), 0) " +
      " FROM T_VENTA v " +
      ...
      "ROUND(SUM(vd.ven_det_cantidad) * 100.0 / NULLIF(SUM(SUM(vd.ven_det_cantidad)) OVER (), 0), 2) AS popularidad " +
      "FROM T_VENTA v " +
      "JOIN T_VENTA_DETALLE vd ON v.ven_id = vd.venta_id " +
      "JOIN T_PRODUCTO p ON vd.producto_id = p.pro_id " +
      "WHERE CAST(v.ven_fecha AS DATE) BETWEEN :fechaInicio AND :fechaFin " +
      "GROUP BY p.pro_id, p.pro_descripcion " +
      "ORDER BY cantidadVendida DESC",
      nativeQuery = true)
  List<Map<String, Object>> obtenerReporteVentasXFecha(@Param("fechaInicio") LocalDate fechaInicio, @Param("fechaFin") LocalDate fechaFin);
}