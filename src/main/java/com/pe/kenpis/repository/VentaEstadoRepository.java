...
public interface VentaEstadoRepository extends JpaRepository<VentaEstadoEntity, Integer> {
  ...
  @Query(value = "SELECT v.ven_id AS venId, " + "       p.pro_tipo AS proTipo, " + "       d.ven_det_cantidad AS venDetCantidad " + "FROM T_VENTA v " + "INNER JOIN T_VENTA_DETALLE d ON v.ven_id = d.venta_id " + "INNER JOIN T_PRODUCTO p ON d.producto_id = p.pro_id " + "INNER JOIN T_VENTA_ESTADO ve ON v.ven_id = ve.venta_id " + "WHERE UPPER(ve.ven_estado) = UPPER(:venEstado) " + "AND CAST(v.ven_fecha AS DATE) = CAST(GETDATE() AS DATE)", nativeQuery = true)
