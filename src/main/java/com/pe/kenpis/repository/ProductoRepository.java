...
public interface ProductoRepository extends JpaRepository<ProductoEntity, Integer> {
  ...
  @Query(value = "WITH ProductosRecursivos AS (" + "    SELECT p.pro_id, p.pro_categoria, p.pro_precio_costo, p.pro_precio_venta, p.pro_descripcion, p.pro_imagen, p.pro_imagen_longitud, p.pro_is_active, p.padre_id, p.emp_id, 1 AS nivel" + "    FROM T_PRODUCTO p" + "    JOIN T_EMPRESA e ON p.emp_id = e.emp_id" + "    WHERE p.pro_id = :categoriaId AND e.emp_is_active = 1" + "    UNION ALL" + "    SELECT p.pro_id, p.pro_categoria, p.pro_precio_costo, p.pro_precio_venta, p.pro_descripcion, p.pro_imagen, p.pro_imagen_longitud, p.pro_is_active, p.padre_id, p.emp_id, pr.nivel + 1" + "    FROM T_PRODUCTO p" + "    JOIN ProductosRecursivos pr ON p.padre_id = pr.pro_id" + "    JOIN T_EMPRESA e ON p.emp_id = e.emp_id" + "    WHERE e.emp_is_active = 1" + ")" + "SELECT * FROM ProductosRecursivos " + "WHERE padre_id = :categoriaId AND pro_is_active = 1",  // Producto activo
      nativeQuery = true)
